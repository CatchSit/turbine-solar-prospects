...
2. **Azure App Registration received and wired into Supabase 2026-08-17** (Authentication → Providers → Azure — Tenant ID `0750b9d3-513f-4030-8191-2d825d3c58f2`, discovered via Microsoft's public OpenID discovery endpoint for `turbineenergyuk.co.uk` since IT's handoff only included the Client ID, Object ID, and secret, not the Tenant ID itself; the Supabase field is labeled "Azure Tenant URL" and expects the full `https://login.microsoftonline.com/<tenant-id>` form, not the bare GUID). **Verified live 2026-08-17**: clicking "Continue with Microsoft" on the real Pages URL correctly redirects to a tenant-branded Microsoft sign-in page with no `AADSTS` error, confirming the Client ID/Tenant/redirect URI are all accepted by Azure. **Not yet fully confirmed:** nobody has completed an actual sign-in with a real `@turbineenergyuk.co.uk` account — do that once before treating this as launched (Section 1 warning still applies until then).
...
3. ~~Get the Azure App Registration back from Turbine Energy's IT team and wire it into Supabase~~ — **done 2026-08-17** (Section 1, item 2). The OAuth handshake is verified as far as it can be without real credentials.
...
5. **Have someone with a real `@turbineenergyuk.co.uk` account complete an actual sign-in end-to-end before telling Turbine Energy staff about the live URL or treating this as launched** — see the warning in Section 1. This is now the only remaining gap in the login path; everything else about the Azure wiring has been verified live.
