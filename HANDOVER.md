...
### `prospect_contacts` table (migration `007`)
...
- **UX gate:** the page also checks `isAdmin(currentUser)` on load (same `ADMIN_EMAILS`/`isAdmin()` helper as `index.html`, now finally wired up — see Section 7 risk 8) and redirects non-admins straight back to `index.html`, so a rep who finds the URL doesn't land on a broken, empty-looking page and wonder if something's wrong.

Three known limitations, all deliberate scope calls made during review rather than bugs — see Section 7, risks 10–12.
...
8. **Auth is now live** (Azure AD via Supabase, matching mcs-map) — see `docs/superpowers/specs/2026-08-12-azure-ad-auth-design.md`. The `ADMIN_EMAILS`/`isAdmin()` stub in `index.html` is **now wired up** — it gates `dashboard.html` (Section 6) alongside RLS, resolved as part of the CRM contact log work (migration `007`, `docs/superpowers/specs/2026-08-17-crm-contact-log-design.md`).
...
10. **`dashboard.html`'s period switcher only affects the two charts.** Switching between "7 days" / "30 days" / "All time" (`periodData()`) re-slices the daily-activity and by-employee charts, but the contact-log table and the CSV export both always render the full, unfiltered `prospect_contacts` result set regardless of which period button is active. This is a deliberate scope call made during review, not a bug — but it means a manager who clicks "7 days" expecting the table (or an export taken right after) to match the chart will be surprised to see every row ever logged. Worth fixing if this becomes a real point of confusion in practice.
11. **`dashboard.html`'s query doesn't filter out soft-deleted rows.** `prospect_contacts` carries `deleted_at`/`deleted_by` columns (Section 5) so a future delete feature can soft-delete without a migration, but there is no delete UI yet (Section 8), so in practice every row currently has `deleted_at IS NULL` and this is moot today. It stops being moot the moment a future task adds delete capability — add a `.is('deleted_at', null)` filter to the dashboard's query at the same time, or soft-deleted contacts will keep showing up in the charts, table, and export.
12. **CSV export has no formula-injection sanitization.** `dashboard.html`'s export builds rows straight from free-text fields (notes, employee name) with only quote-escaping, no check for a leading `=`, `+`, `-`, or `@`. If a note field starts with one of those characters and the exported file is opened in Excel or Google Sheets, that cell can be interpreted as a formula rather than plain text. Low severity here — this is an internal, manager-only export of a table nobody outside Turbine Energy can even read (RLS, Section 5) — but worth hardening (prefix a `'` or strip leading special characters before quoting) if this export is ever shared more broadly than the manager's own machine.
