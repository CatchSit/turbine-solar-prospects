...
### Step 7 — Company sector & maturity classification (optional, additive — **built and deployed 2026-08-20, not yet run**)
```
COMPANIES_HOUSE_API_KEY=... SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npm run classify-companies
```
Unlike `solar-enrichment`, this is a **plain script, not an edge function** — Companies House has no monthly cap (only a 600-requests/5-min rate limit), so the whole 21,808-prospect pass runs to completion in one long sitting instead of needing repeated manual invocations spread over months. For each prospect with a postcode and no existing `company_classifications` row: searches Companies House, ranks candidates by the same building-number address match `company-lookup` uses, fetches the top match's profile (SIC codes, incorporation date, accounts-filing status), and upserts one row. Idempotent — safe to re-run; only processes prospects still missing a row. **Expect roughly 6-8 hours** at ~2 calls/prospect paced under the rate limit — run it in the background, not something to wait on. Full design in `docs/superpowers/specs/2026-08-20-company-sector-maturity-classification-design.md`.

**Status: code shipped and live (migration `012`, `company-lookup` deployed with the new `accounts_type` field, all frontend sector/maturity filters and popup tags/badges verified against fixture data in a real browser) — the actual classification run itself has not been started yet.** It needs `COMPANIES_HOUSE_API_KEY` and `SUPABASE_SERVICE_ROLE_KEY` available as local environment variables to run, which weren't available in the session that built this (see Section 9 "Next steps"). Until it runs, every prospect's "Business sector" and "Company maturity" sidebar filters will show 0 for every real bucket and no popup tags will appear — same "expected, not a bug" situation as the EPC recommendations step above before its CSV was downloaded. **Also verify `accounts_type`'s field path (`json.accounts?.last_accounts?.type`) against a real Companies House response in the first few minutes of running this** — it's an unverified guess at Companies House's response shape (see `supabase/functions/company-lookup/index.ts`'s `fetchProfile()` and the matching note in `classify-companies.mjs`); if `accounts_type` comes back null for real companies that should have filed accounts, the field path needs correcting in both files before letting the full run continue.
...
**Already done:**
- Repo created and pushed to `main` (https://github.com/CatchSit/turbine-solar-prospects); Supabase project `turbine-solar-prospects` created, migrations 001–013 applied. **Note: live prospect count is now 22,016, not the 21,808 figure quoted elsewhere in this doc** — grown since that figure was last measured; treat the live count as authoritative and update this doc's older references opportunistically rather than assuming 21,808 is still exact.
...
- Popup relevance ranking + tabbed layout (2026-08-20, `docs/superpowers/specs/2026-08-20-popup-relevance-and-tabs-design.md`) — companies badged "Likely this building," popup restructured into Business Rates/Companies tabs, whitespace tightened, card widened to 360px. Live-verified in a real browser against fixture data; real-session live checklist still pending (Section 6).
- Company sector & maturity classification built (migration 012, `company-lookup` extended with `accounts_type`, `scripts/classify-companies.mjs`, sidebar filters, popup tags/badges) — **code shipped and deployed 2026-08-20, classification run started same day** (Section 4, Step 7) — sidebar Business Sector/Company Maturity counts and popup tags now populating live as the multi-hour run progresses.
- VOA as a primary prospect source, not just enrichment (migration 013, `scripts/ingest-business-rates.mjs` extended, `scripts/geocode-postcodes.mjs` backfills `local_authority`, EPC Rating filter's null-passthrough bug fixed pre-emptively) — **code shipped and deployed 2026-08-20, not yet run** (Section 4, Step 6) — no new prospects seeded yet.
- Proposal file attachments built (migrations 014/015, `prospect_proposals` table + `proposals` Storage bucket with server-side size/type limits, new Proposals popup tab — Section 5/6) — any authenticated `@turbineenergyuk.co.uk` rep can upload and reopen a PDF proposal against a prospect. **Built and reviewed 2026-08-21, including a final cross-cutting fix pass** — real end-to-end live verification with a signed-in session is still a pending manual step (Task 5 of `docs/superpowers/plans/2026-08-21-proposal-attachments.md`, outside this branch's automated work).
...
8. **Run `npm run classify-companies`** (Section 4, Step 7) with `COMPANIES_HOUSE_API_KEY`/`SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` set — needs local credentials that weren't available in the session that built this, so it's the one piece of this feature not yet actually run. Expect ~6-8 hours; verify `accounts_type` against a real response in the first few minutes (Section 4, Step 7 has the details) before letting it run to completion. Once it's made real progress, spot-check the "Business sector" sidebar filter's chip counts and a popup on a known multi-tenant postcode to confirm the buckets look sensible — adjust `shared/sic-sector-config.js` if not (pure client-side config, no re-run needed).
9. Live-verify the popup relevance/tabs work (item above) against a real signed-in session — the checklist is in `docs/superpowers/plans/2026-08-20-popup-relevance-and-tabs.md`, Task 3, Step 3.
10. **Run `DRY_RUN=1 npm run ingest-business-rates`** (Section 4, Step 6) with `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` set — reports how many new VOA-sourced prospects would be created with zero writes. Check the count looks reasonable, then run again without `DRY_RUN=1` for real, then re-run `npm run geocode` to place the new prospects on the map. Same local-credentials blocker as item 8 above — not run yet.
