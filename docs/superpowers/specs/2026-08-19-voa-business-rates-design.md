...
- New sidebar range filter, **"Rateable value (£)"**, mirroring the existing floor-area min/max exactly in UI and behavior. A prospect passes the filter if **any** of its hereditaments' rateable value falls in range (not the sum) — consistent with showing them separately, and avoiding one large tenant plus several small ones averaging into something misleading.
...
- Confirm a prospect with multiple hereditaments renders each one separately in the popup, and that the range filter's "any hereditament in range" semantics work correctly against a multi-hereditament prospect.
